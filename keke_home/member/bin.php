...
/*	$jd =dbmysql::getInstance($con_db_host,'shop','t2B2A4m8','shop');
	...
	if(!isset($_COOKIE['shoumi'])){
		...
		if($bFlag){
			writeLog("shoumi success update sql:" . $sql3);
		}
	...
	if($memberLogged){
		...
		if(strlen($id) <= 0){
			$query = "select count(id) as sum from `h_member` where h_parentUserName = '{$memberLogged_userName}'";
			$result = $db->get_one($query);
			$iNum = intval($result['sum']);
			echo '{';
			echo 'id:"' , $memberLogged_userName , '"';
			echo ', pId:"0"';
			echo ', name:"[0] ' , $memberLogged_userName . '|<span style=\"color:#4CAF50;font-weight:800;\">直推总人数:' . $iNum , ' [' , ($memberLogged_isPass?'√激活':'×未激活') , '] "';
			echo ', isParent:true';
			...
			while($rs_list = $db->fetch_array($result)){
				...
				echo ', name:"[' , ($lv + 1) , '] ' , $rs_list['h_userName'] . '|<span style=\"color:#4CAF50;font-weight:800;\">注册时间:' . $rs_list['h_regTime'] . '|  直推总人数:' . $rs_list['comMembers'] . '</span>', ' [' , ($rs_list['h_isPass']?'√激活':'×未激活') , ']"';
